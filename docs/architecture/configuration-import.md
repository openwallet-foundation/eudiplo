...
- **Key chain management**: Unified key chains (keys + certificates) are stored in the `key-chains/` directory and
  will be imported automatically. Keep a provisioning file in place while it is
  intended to own the resource. Some KMS providers do not permit private-key
  export; portable resources use an external KMS reference in that case.
- **Ownership**: A successfully provisioned resource is marked `file-managed`.
  API and web-client edits are rejected until the resource is explicitly
  detached. See [Configuration Portability and Versioning](configuration-portability.md).
- **Versioning**: Bare JSON is accepted for backward compatibility. Versioned
  resources use the `apiVersion`, `kind`, `metadata`, and `spec` envelope and
  are upgraded through sequential config migrations before validation.
...
- **Checks the startup mode** - Imports unless `CONFIG_IMPORT_MODE=disabled`
- **Scans tenant directories** - Processes each tenant folder independently
- **Builds a versioned bundle** - Wraps legacy JSON and runs schema migrations
- **Plans the change** - Validates generations, references, and ownership
- **Applies in dependency order** - Uses the same plan/apply pipeline as API
  bundle imports

### Startup reconciliation mode

Use one setting for both enablement and duplicate handling:
...
|---|---|
| `disabled` | Do not scan or apply tenant configuration folders |
| `create` | Create missing resources and skip existing resources without adopting them |
| `upsert` | Create missing resources and update existing resources from the folder |
| `replace` | Upsert, then remove resources previously managed by the same tenant folder but no longer present |

`replace` never deletes the tenant itself and does not prune resources owned by
another bundle or folder source.

`CONFIG_IMPORT` and `CONFIG_IMPORT_FORCE` remain available for one compatibility
cycle. When `CONFIG_IMPORT_MODE` is unset, they map as follows and emit a
deprecation warning:

| Legacy settings | Effective mode |
|---|---|
| `CONFIG_IMPORT=false` | `disabled` |
| `CONFIG_IMPORT=true`, `CONFIG_IMPORT_FORCE=false` | `create` |
| `CONFIG_IMPORT=true`, `CONFIG_IMPORT_FORCE=true` | `upsert` |

### Validating Before Deployment with the CLI

Configuration errors are easier to detect before EUDIPLO starts, especially in
CI/CD pipelines, Kubernetes deployments, and multi-tenant installations. The
[EUDIPLO CLI](https://www.npmjs.com/package/@eudiplo/cli) can validate tenant
config-import files without starting the backend, connecting to a database, or
changing any deployment state:

```bash
# Validate a single tenant directory
eudiplo config validate tenant ./assets/config/root

# Validate every tenant under a configuration root
...
- Discovers tenant directories and requires a valid `info.json` for each one.
- Validates every supported resource type (tenant metadata, clients, key
  chains, credential configs, issuance configs, presentation configs, status
  lists, trust lists, attribute providers, webhook endpoints, registrar
  config, and tenant-specific KMS config) against the same JSON Schemas the
  backend uses for config-import.
- Detects unresolved `${VAR}` placeholders without a default, without ever
  printing resolved secret values.
- Collects errors across every file and tenant instead of stopping at the
  first problem, and reports the tenant, filename, and property path for each
  error.
...
regenerated by `pnpm run gen:api`.

Consumers can install the same bundled schemas and scoped VS Code associations
...
This provides completion and inline validation while authoring files;
`eudiplo config validate tenants ./config` remains the editor-independent and
CI-friendly validation step.
...
- **Duplicate handling** - Uses the selected `CONFIG_IMPORT_MODE`
...
- **Existing configurations** are preserved in `create` mode
- **Plan failures** prevent resource changes from that tenant's bundle; tenant
  setup may already have created the tenant itself
- **Apply-time failures** are reported with the tenant and stop the remainder of
  that tenant's reconciliation
- **Other tenants continue** when one tenant folder fails
...
- **Use `CONFIG_IMPORT_MODE=create`** for additive initial provisioning
- **Use `upsert` or `replace`** only when the folder should remain authoritative
- **Monitor logs** for validation errors during startup
- **Detach file-managed resources** before managing them through the API or UI
...
**Import not running**: Check `CONFIG_IMPORT_MODE` is not `disabled`

**Configurations not updating**: Select `CONFIG_IMPORT_MODE=upsert` and increase
the resource generation when required
