...
Use the migration helper to scan JSON configs for incompatible entries:

```bash
pnpm run migrate:trust-lists:v7
```

You can also scope the scan to specific paths:

```bash
node scripts/migration-guide-v7-trust-list.mjs ./config ./deployment
```

The script exits with code `1` when incompatible entries are detected.
