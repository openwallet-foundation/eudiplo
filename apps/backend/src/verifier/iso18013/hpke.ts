...
const hpkeSuiteId = concat(enc.encode('HPKE'), i2osp(KEM_ID, 2), i2osp(KDF_ID, 2), i2osp(AEAD_ID, 2));
...
function hkdfExtract(salt: Buffer | null, ikm: Buffer): Buffer {
    const realSalt = salt && salt.length > 0 ? salt : Buffer.alloc(SHA256_LEN, 0);
...
function hkdfExpand(prk: Buffer, info: Buffer, len: number): Buffer {
    ...
    for (let n = 1; written < len; n++) {
        ...
        h.update(t); h.update(info); h.update(Buffer.from([n]));
        t = h.digest();
...
function labeledExtract(suiteId: Buffer, salt: Buffer | null, label: string, ikm: Buffer): Buffer {
    const labeled = concat(enc.encode('HPKE-v1'), suiteId, enc.encode(label), ikm);
    return hkdfExtract(salt, labeled);
}

// RFC 9180 §4 LabeledExpand
function labeledExpand(suiteId: Buffer, prk: Buffer, label: string, info: Buffer, len: number): Buffer {
    const labeledInfo = concat(i2osp(len, 2), enc.encode('HPKE-v1'), suiteId, enc.encode(label), info);
 ...
 * RFC 9180 §4.1
 */
function dhkemDecap(encKey: Buffer, recipientPrivateKeyJwk: { x: string; y: string; d: string }): Buffer {
    ...
    const prk = labeledExtract(kemSuiteId, Buffer.alloc(SHA256_LEN, 0), 'shared_secret', dhOutput);
 ...
 */
function keySchedule(sharedSecret: Buffer, info: Buffer): { key: Buffer; baseNonce: Buffer } {
    const mode = Buffer.from([0x00]); // base mode

    // psk_id_hash = LabeledExtract("", "psk_id_hash", "")
    const pskIdHash = labeledExtract(hpkeSuiteId, null, 'psk_id_hash', Buffer.alloc(0));
    ...
    const secret = labeledExtract(hpkeSuiteId, sharedSecret, 'secret', Buffer.alloc(0));
    ...
    const baseNonce = labeledExpand(hpkeSuiteId, secret, 'base_nonce', ksContext, Nn);
